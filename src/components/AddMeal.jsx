...
const AddMeal = ({ setIsOpen }) => {
  ...
  const handleAddMeal = async () => {
    ...
    try {
      setIsError(false)
      const meal = {
        name, source, ingredients: ingredientsStr.split(','), last_eaten: format(new Date(lastEaten), 'yyyy/MM/dd'), rating, image, created_by: loggedInUser.username
      }
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={image}
              onChange={handleInputChange(setImage)} // Handle change
              className="mt-1 p-2 w-full border rounded-md focus:ring-indigo-500 focus:border-indigo-500"
            />
          </div>
        </div>
